...
app.get("/", (req, res) => {
  // GET HEROES
  let query = `SELECT heroes_tb.id,heroes_tb.name,heroes_tb.photo,type_tb.name AS type_name FROM heroes_tb INNER JOIN type_tb ON type_id = type_tb.id ORDER BY heroes_tb.id DESC`;
...
app.post("/add-type", (req, res, next) => {
  ...
  if (name.length === 0) {
    ...
    let query = ` INSERT INTO type_tb (name) VALUES ("${name}")`;
    dbConnection.query(query, (err, rows) => {
      if (err) throw err;
      res.redirect("/");
    });
...
app.get("/detail-hero/(:id)", (req, res) => {
  let id = req.params.id;
  // let query = `SELECT * FROM heroes_tb WHERE id = ${id}`;
  let query = `SELECT heroes_tb.id,heroes_tb.name,heroes_tb.photo,type_tb.name AS type_name FROM heroes_tb  INNER JOIN type_tb ON type_id = type_tb.id WHERE heroes_tb.id = ${id}`;
